# Lessons Learned Agent Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use executing-plans to implement this plan task-by-task.

**Goal:** Create a lessons-learned agent that analyzes project history (git + conversations) and maintains CLAUDE.md quality automatically.

**Architecture:** Markdown-based agent definition following the existing agent pattern (web-research, codebase-research, context-indexing). Uses Sonnet model for sophisticated pattern recognition and critical documentation updates. No external scripts needed - all logic embedded in agent prompt.

**Tech Stack:** Claude Code agent system, Markdown (YAML frontmatter), Bash tools

---

## Task 1: Create Agent File Structure

**Files:**
- Create: `ai-assisted-development/agents/lessons-learned.md`
...
Run: `claude plugin validate ai-assisted-development/`
Expected: No errors in frontmatter structure

**Step 3: Commit**

```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: create lessons-learned agent file structure"
```

---

## Task 2: Add Initialization and Scope Determination

**Files:**
- Modify: `ai-assisted-development/agents/lessons-learned.md`

**Step 1: Add Step 1 to workflow**
...
```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: add initialization and scope determination step"
```

---

## Task 3: Add Git Evidence Gathering

**Files:**
- Modify: `ai-assisted-development/agents/lessons-learned.md`

**Step 1: Add Step 2 to workflow**
...
```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: add git evidence gathering step"
```

---

## Task 4: Add Conversation Evidence Gathering

**Files:**
- Modify: `ai-assisted-development/agents/lessons-learned.md`

**Step 1: Add Step 3 to workflow**
...
```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: add conversation evidence gathering step"
```

---

## Task 5: Add Pattern Extraction and Filtering

**Files:**
- Modify: `ai-assisted-development/agents/lessons-learned.md`

**Step 1: Add Step 4 to workflow**
...
```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: add pattern extraction and filtering step"
```

---

## Task 6: Add CLAUDE.md Inconsistency Detection

**Files:**
- Modify: `ai-assisted-development/agents/lessons-learned.md`

**Step 1: Add Step 5 to workflow**
...
```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: add CLAUDE.md inconsistency detection step"
```

---

## Task 7: Add CLAUDE.md Update Logic

**Files:**
- Modify: `ai-assisted-development/agents/lessons-learned.md`

**Step 1: Add Step 6 to workflow**
...
```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: add CLAUDE.md update logic step"
```

---

## Task 8: Add Reporting and Summary

**Files:**
- Modify: `ai-assisted-development/agents/lessons-learned.md`

**Step 1: Add Step 7 to workflow**
...
```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: add reporting and summary step"
```

---

## Task 9: Add Context Engineering Principles and Error Handling

**Files:**
- Modify: `ai-assisted-development/agents/lessons-learned.md`

**Step 1: Add context engineering guidance section**

Insert after "## Complete Workflow" and before "### Step 1":
...
```bash
git add ai-assisted-development/agents/lessons-learned.md
git commit -m "feat: add context engineering principles and error handling"
```

---

## Task 10: Register Agent in Plugin

**Files:**
- Modify: `ai-assisted-development/.claude-plugin/plugin.json`

**Step 1: Add agent to agents array**

In plugin.json, update the "agents" array:
...
Run: `claude plugin validate ai-assisted-development/`
Expected: No errors, agent recognized

**Step 3: Commit**

```bash
git add ai-assisted-development/.claude-plugin/plugin.json
git commit -m "feat: register lessons-learned agent in plugin"
```

---

## Task 11: Test the Agent
...
Or install fresh:
```bash
claude plugin install ai-assisted-development/
```

**Step 2: Test basic invocation**

Test in a project with some git history:
```bash
# This would be done through Claude Code interface, not command line
# User would invoke the agent via Task tool with subagent_type='lessons-learned'
```

Expected behavior:
1. Agent analyzes git history
2. Attempts to find conversation history (may be unavailable)
3. Extracts patterns
4. Reports findings or "no new learnings identified"

**Step 3: Verify output quality**

Check that agent:
- Cites specific commit hashes
- Provides actionable guidance
- Doesn't add generic/vague learnings
- Properly categorizes patterns

**Step 4: Document test results**

Create test summary noting:
- Whether agent successfully analyzed history
- Quality of identified patterns
- Any errors or issues encountered
- Performance (how long it took)
...
- [ ] Context engineering principles section included
...
- [ ] Agent can be invoked through Task tool
- [ ] Agent produces reasonable output on test project
...
**Why Sonnet model:**
- Requires sophisticated pattern recognition across complex diffs
- Needs nuanced judgment about contradictions and quality
- Must understand conversation context and extract implicit learnings
- Critical task where errors could corrupt important documentation

**Testing strategy:**
- Initial tests on this repository (claude-code-toolbox) since it has rich history
- Verify agent handles edge cases: no CLAUDE.md, no conversations, very old CLAUDE.md
- Check that learnings are actually project-specific, not generic

**Future enhancements:**
- Add support for analyzing multiple projects simultaneously
- Create learning categories taxonomy
- Add confidence scores to learnings
- Support for custom learning pattern matchers
